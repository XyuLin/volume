...
class Route
{
    ...
    public static function rule($rule, $route = '', $type = '*', $option = [], $pattern = [])
    {
        ...
        if (!is_null($group)) {
            // 路由分组
            $option  = array_merge(self::getGroup('options'), $option);
    ...
    protected static function setRule($rule, $route, $type = '*', $option = [], $pattern = [], $group = '')
    {
        ...
        if ($group) {
            ...
            if (self::$domain) {
                self::$rules['domain'][self::$domain]['*'][$group]['rule'][] = ['rule' => $rule, 'route' => $route, 'var' => $vars, 'options' => $option, 'pattern' => $pattern];
            } else {
                self::$rules['*'][$group]['rule'][] = ['rule' => $rule, 'route' => $route, 'var' => $vars, 'options' => $option, 'pattern' => $pattern];
            }
        } else {
            ...
            if (self::$domain) {
                self::$rules['domain'][self::$domain][$type][$rule] = ['rule' => $rule, 'route' => $route, 'var' => $vars, 'options' => $option, 'pattern' => $pattern];
            } else {
                self::$rules[$type][$rule] = ['rule' => $rule, 'route' => $route, 'var' => $vars, 'options' => $option, 'pattern' => $pattern];
            }
            if ('*' == $type) {
                // 注册路由快捷方式
     ...
     * 获取当前的分组信息
     * @access public
     * @param string $type 分组信息名称 name options pattern
     * @return mixed
     */
    ...
    public static function group($name, $routes, $option = [], $pattern = [])
    {
        ...
        if (!empty($name)) {
            if ($routes instanceof \Closure) {
                $currentOption  = self::getGroup('options');
                ...
                if ($currentGroup != $name) {
                    ...
                    self::$rules['*'][$name]['options']  = $option;
                ...
                foreach ($routes as $key => $val) {
                    ...
                    $item[] = ['rule' => $key, 'route' => $route, 'var' => $vars, 'options' => $options, 'pattern' => $patterns];
                    // 设置路由标识
                    $suffix = isset($options['ext']) ? $options['ext'] : null;
                    self::name($route, [$name . ($key ? '/' . $key : ''), $vars, self::$domain, $suffix]);
                }
                self::$rules['*'][$name] = ['rule' => $item, 'route' => '', 'var' => [], 'options' => $option, 'pattern' => $pattern];
            }
    ...
    public static function check($request, $url, $depr = '/', $checkDomain = false)
    {
        ...
        if (isset($rules[$item])) {
            ...
            if (!empty($rule['route']) && self::checkOption($rule['options'], $request)) {
                self::setOption($rule['options']);
                return self::parseRule($item, $rule['route'], $url, $rule['options']);
            }
        }
    ...
    private static function checkRoute($request, $rules, $url, $depr = '/', $group = '', $options = [])
    {
        ...
        if (isset($auto)) {
            ...
            // 未匹配所有路由的路由规则处理
            return self::parseRule('', $miss['route'], $url, $miss['options']);
        }
        return false;
    }
    ...
    private static function parseRule($rule, $route, $pathinfo, $option = [], $matches = [], $fromCache = false)
    {
        ...
        // 记录匹配的路由信息
        $request->routeInfo(['rule' => $rule, 'route' => $route, 'options' => $option, 'var' => $matches]);
