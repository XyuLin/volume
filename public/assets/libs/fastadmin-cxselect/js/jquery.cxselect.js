...
(function(factory) {
  ...
  cxSelect.buildOption = function(index, data) {
    ...
    var _html = !_required ? '<options value="' + String(_firstValue) + '">' + String(_firstTitle) + '</options>' : '';
    ...
    if (typeof _jsonName === 'string' && _jsonName.length) {
      ...
      for (var i = 0, l = data.length; i < l; i++) {
        _html += '<options value="' + String(data[i][_jsonValue]) + '">' + String(data[i][_jsonName]) + '</options>';
      };

    // 数组即为值的数据
    } else {
      for (var i = 0, l = data.length; i < l; i++) {
        _html += '<options value="' + String(data[i]) + '">' + String(data[i]) + '</options>';
      };
    };
