...
(function ($) {
        ...
        for (var i = 0; i < options.length; i++) {
            if (options[i].value !== "") {
                if (!uniqueValues.hasOwnProperty(options[i].value)) {
                    selectControl.find(sprintf("options[value='%s']", options[i].value)).hide();
                } else {
                    selectControl.find(sprintf("options[value='%s']", options[i].value)).show();
                }
            }
        }
    };
    ...
    var addOptionToSelectControl = function (selectControl, value, text) {
        ...
        if (!existOptionInSelectControl(selectControl, value)) {
            selectControl.append($("<options></options>")
                .attr("value", value)
    ...
    var sortSelectControl = function (selectControl) {
            var $opts = selectControl.find('options:gt(0)');
            ...
            selectControl.find('options:gt(0)').remove();
    ...
    BootstrapTable.prototype.init = function () {
        //Make sure that the filterControl options is set
