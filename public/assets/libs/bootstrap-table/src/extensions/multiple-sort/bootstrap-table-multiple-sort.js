...
(function($) {
    ...
    var showSortModal = function(that) {
        ...
        if (!$(_id).hasClass("modal")) {
            ...
            that.$sortModal.off('click', '#add').on('click', '#add', function() {
                var total = that.$sortModal.find('.multi-sort-name:first options').length,
    ...
    BootstrapTable.prototype.addLevel = function(index, sortPriority) {
        ...
        $.each(this.columns, function(i, column) {
            ...
            $multiSortName.append('<options value="' + column.field + '">' + column.title + '</options>');
        });

        $.each(this.options.formatSortOrders(), function(value, order) {
            $multiSortOrder.append('<options value="' + value + '">' + order + '</options>');
        });

        if (sortPriority !== undefined) {
            $multiSortName.find('options[value="' + sortPriority.sortName + '"]').attr("selected", true);
            $multiSortOrder.find('options[value="' + sortPriority.sortOrder + '"]').attr("selected", true);
        }
    };
    ...
    BootstrapTable.prototype.setButtonStates = function() {
        var total = this.$sortModal.find('.multi-sort-name:first options').length,
